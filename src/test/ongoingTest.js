...
class ongoingTest extends Component {
  ...
  submitTest() {
    ...
    axios.post(`https://classcast-198812.appspot.com/white_label/store_test_performance/`, this.state.blocks)
      .then((response) => {
      ...
      return(
        <View style={styles.container}>
          ...
          <ScrollView style={{width: '100%'}}>
            ...
            { 
              this.props.navigation.state.params.data.sections[this.state.selectedSection].blocks[this.state.questionIndex].options && this.props.navigation.state.params.data.sections[this.state.selectedSection].blocks[this.state.questionIndex].options.map((options, index)=>{
                return(
                  <TouchableNativeFeedback
                    onPress={() => {
